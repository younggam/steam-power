...
const matterDisintegrator=extendContent(GenericCrafter,"matter-disintegrator",{
  itemValues:[],
  tmpImage:null,
  pauseImg:null,
  playImg:null,
  ...
  init(){
    ...
    Vars.content.items().each(cons(item=>{
      ...
      this.itemValues[item.id]=Math.max(value,0.1)
    }));
  },
  ...
  update(tile){
    ...
    if(entity.progress==0){
      ...
      entity.progress=0;
      entity.setCurrentItem(null);
    }else entity.progress+=this.getProgressIncrease(entity,this.craftTime);
  },
  buildCommon(entity){
    var cb=Core.bundle
    ...
    if(this.dialog1==null){
      ...
      valueTable.table(cons(t=>{
        ...
        for(var i=0;i<this.itemValues.length;i++){
          if(this._containerItemMap.get(i)!=null){ continue;}
    ...
    cont.addImage().height(4).pad(6).color(Pal.gray).growX().width(300).row();
    cont.table(cons(tt=>{
      tt.add(new Label("")).update(cons(e=>e.setText(display2+" x"+(entity.power.status*entity.timeScale).toFixed(2)))).padRight(8);
...
matterDisintegrator.entityType=prov(()=>extend(GenericCrafter.GenericCrafterEntity,{
  _pause:false,
  ...
  _heat:25,
  delta(){
    return Time.delta()*(this.timeScale+Math.max(0,this._heat/1000-2))
  },
}));
matterDisintegrator.configurable=true;
matterDisintegrator.consumes.powerCond(60,boolf(e=>{
  if(typeof e["getCurrentItem"]==="function") return e.getCurrentItem()!=null&&!e.isPaused()
  return false;
}));;
